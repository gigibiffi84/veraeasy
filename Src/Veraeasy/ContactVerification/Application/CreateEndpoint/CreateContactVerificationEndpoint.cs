...
{
    private static async Task<Guid> DoCreateContactVerification(CreateContactVerificationRequest request,
    ...
    {
        app.MapPost(ContactVerificationApiPaths.CreateOrUpdate,
                async
                (
                ...
                {
                    var contractId = await DoCreateContactVerification(request, aggregate, user, cancellationToken);
